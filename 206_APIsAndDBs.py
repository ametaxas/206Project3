...
try:
	cache_file = open(CACHE_FNAME, 'r') #looks for a cache file
	cache_contents = cache_file.read()
	cache_diction = json.loads(cache_contents) #and loads its contents into the cache dictionary
	cache_file.close()
except: #if there is no file
	cache_diction = {'timeline_info':{}, 'user_info':{}} #initalize a dictionary

# Define your function get_user_tweets here:
def get_user_tweets(user):
	if user in cache_diction['timeline_info']: #if the user is in the cahce dictionary key that contains timeline info
		print ('using cache') #print
	else:
		print ('retreving from the internet') #if the users is not in the cache dictionary key that contains timeline info
		timeline = api.user_timeline(user, count = 20) #access the timeline info of the user
		cache_diction['timeline_info'][user] = timeline #add it to the cache key that contains timeline info
		cache_file = open(CACHE_FNAME, 'w') 
		cache_file.write(json.dumps(cache_diction, indent = 4)) #and write the new info into the cache file
		cache_file.close()
	return cache_diction['timeline_info'][user] #return the list of tweets of the user
...
cur.execute('DROP TABLE IF EXISTS Users') #removes the table if it exists so we can create the table over and over again without causing an error
cur.execute('CREATE TABLE Users (user_id TEXT NOT NULL PRIMARY KEY, screen_name TEXT, num_favs INTEGER, description TEXT)') #creates the table and specifies the type of each column

mentioned_users = [88836132] #initalizes a list that contains the umich id 
for tweet in umich_tweets: 
	for user in tweet['entities']['user_mentions']: #if a user is mentioned in umich's tweets
		if user['id'] not in mentioned_users: #and they are not already in the list 
			mentioned_users.append((user['id'])) #add their id to the list

for user in mentioned_users: 
	if user not in cache_diction['user_info']: #if any mentioned user's info isnt already in the cache
		cache_diction['user_info'][user] = api.get_user(user) #access the mentioned users info and add it to the key in the cache dictionary that contains user info
		cache_file = open(CACHE_FNAME, 'w')
		cache_file.write(json.dumps(cache_diction, indent = 4)) #and write the new info to the cache file
		cache_file.close()
	user_info = cache_diction['user_info'][user]
	table_info = (user_info['id_str'], user_info['screen_name'], user_info['favourites_count'], user_info['description']) #navigates through the nested dictionary of user info to find desired variables
	cur.execute('INSERT INTO Users (user_id, screen_name, num_favs, description) VALUES (?,?,?,?)', table_info) #puts desired variables into the table columns
...
cur.execute('CREATE TABLE Tweets (tweet_id TEXT NOT NULL PRIMARY KEY, text TEXT, user_posted TEXT references Users(user_id), time_posted DATETIME, retweets INTEGER)') #creates the table and specifies the type of each column

for tweet in umich_tweets:
	tweet_info = (tweet['id_str'], tweet['text'], tweet['user']['id_str'], tweet['created_at'], tweet['retweet_count']) #navigates through the nested dictionary of tweet info to find desired variables
	cur.execute('INSERT INTO Tweets (tweet_id, text, user_posted, time_posted, retweets) VALUES (?,?,?,?,?)', tweet_info) #puts desired variables into the table columns

conn.commit()	#commits the info into the table
...
users_info = [tup for tup in cur] #accumulates a list of tuples from the cursor
...
screen_names = [name[0] for name in cur] #accumulates a list of strings from the cursor
# Make a query to select all of the tweets (full rows of tweet information)
# that have been retweeted more than 10 times. Save the result 
# (a list of tuples, or an empty list) in a variable called retweets.
cur.execute('SELECT tweet_id, text, user_posted, time_posted, retweets FROM Tweets WHERE retweets > 10') #selects from tweets with the condition of more than 10 retweets
retweets = [tup for tup in cur] #accumulates a list of tuples from the cursor 
...
cur.execute('SELECT description FROM Users WHERE num_favs > 500') #selects from users with the condition of more than 500 favories
favorites = [description[0] for description in cur] #accumulates a list of strings from the cursor
# Make a query using an INNER JOIN to get a list of tuples with 2 
# elements in each tuple: the user screenname and the text of the 
# tweet. Save the resulting list of tuples in a variable called joined_data2.
cur.execute('SELECT Users.screen_name, Tweets.text FROM Users INNER JOIN Tweets ON Tweets.user_posted = Users.user_id') #combines information from two tables
joined_data = [tup for tup in cur] #acucumulates a list of tuples from the cursor
# Make a query using an INNER JOIN to get a list of tuples with 2 
# elements in each tuple: the user screenname and the text of the 
# tweet in descending order based on retweets. Save the resulting 
# list of tuples in a variable called joined_data2.
cur.execute('SELECT Users.screen_name, Tweets.text FROM Tweets INNER JOIN Users ON Tweets.user_posted = Users.user_id ORDER BY retweets DESC') #combines information from two tables with an ordering condition
joined_data2 = [tup for tup in cur] #accumulates a list of tuples from the cursor
